...
(async () => {
    ...
    for (const objId of Object.keys(enabledDps)) {
        console.log(`Start migration of ${objId}`);

        const migratedValues = await runMigrationOf(objId);
        if (deleteOldValuesInHistory && migratedValues > 0) {
            await sendToAsync('history.0', 'deleteAll', [{ id: objId }]); // delete values
            await sendToAsync('history.0', 'disableHistory', { id: objId }); // disable logging to history
        }
    }
...
async function runMigrationOf(objId) {
